"""
    ...
    @Description: process Dataset
        2021/7/12 Reconstruction: reduce code
        2021/10/12 Easy reconstruction
...
class DataLoader:
    ...
    def __init__(self, dataset: Dataset, batch_size: int, image_size=(256, 256), shuffle: bool = True,
                 is_video: bool = True):
        """
        Data Loader: get all data
        :param dataset: train data
        :param batch_size: batch size
        :param image_size: obtained image size default (256, 256)
        :param shuffle: if shuffle default True
        """
        self.trains = list()
        self.labels = list()
    ...
    def build(self):
        """
        build our dataset with batch_size
        :return:
        """
        ...
        for i in range(items):
            image_list, labels = self.dataset.trains[i * self.batch_size:(i + 1) * self.batch_size], \
                                 self.dataset.labels[i * self.batch_size:(i + 1) * self.batch_size]
    ...
    def getBatchImage(self, file_path_list, y_labels):
        ...
        for file_path, label in zip(file_path_list, y_labels):
            ...
            if self.is_video and len(image) == 0:
                ...
                # flip , rotate , movition 传numpy进去
                image = self.imageAugement(image, mode="resize",train=False)
